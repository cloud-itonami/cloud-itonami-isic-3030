(ns aerospace.operation
  "OperationActor -- one aerospace-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Aerospace Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Aerospace Manufacturing
  ...
  One graph run = one aerospace-manufacturing operation (intake ->
  ...
  decision to a human operator (the manufacturing engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-assembly`/`:actuation/issue-
  airworthiness-evidence` ALWAYS reach this node when the governor is
  clean -- see `aerospace.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [aerospace.aerospaceadvisor :as aerospaceadvisor]
...
(defn build
  ...
  `aerospace.store/Store`).
  opts:
    :advisor      -- an `aerospace.aerospaceadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (aerospaceadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Aerospace Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (aerospaceadvisor/-advise advisor store request)]
            {:proposal p :audit [(aerospaceadvisor/trace request p)]})))

      ;; Aerospace Manufacturing Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
